    ...
    parseExpression tokens = 
        case lookAhead tokens of
            ...
            LeftParenToken -> let (expr, tokens') = parseExpression (accept tokens)
                            in
                                let rightParen = lookAhead tokens'
                                    in 
                                        case rightParen of
                                            RightParenToken -> (ParenthesizeNode expr, accept tokens')
            ...
            -- This last section was meant to be an error out token to make sure that the function can do that.
            _ -> (EmptyNode, tokens) 
